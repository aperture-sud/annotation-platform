...
export default function AdminPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f6f8' }}>
      ...
      {reviewModal && (
        <div
          style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}
                  ...
                  {allGroups.map((group) => {
                    ...
                    <div key={group.username}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '10px', fontWeight: 600, backgroundColor: '#f0f0f0', color: '#555' }}>
                          {total} uploaded
                        </span>
                        <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '10px', fontWeight: 600, backgroundColor: '#fff8e1', color: '#e65100' }}>
                          {needsReview} pending
                        </span>
                        <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '10px', fontWeight: 600, backgroundColor: '#e8f5e9', color: '#2e7d32' }}>
                          {group.approved} approved
                        </span>
                        <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '10px', fontWeight: 600, backgroundColor: '#fce4ec', color: '#b71c1c' }}>
                          {group.flagged} flagged
                        </span>
                        {group.uploads.length > 0 && (
                          <span style={{ fontSize: '12px', color: '#aaa' }}>{expandedUser === group.username ? '▲' : '▼'}</span>
                        )}
                      </button>

                      {expandedUser === group.username && (
                        <div style={{ marginTop: '6px', display: 'flex', flexWrap: 'wrap', gap: '8px', padding: '4px 2px 8px' }}>
                          {group.uploads.map((up) => {
                            const status = up.upload_approval_status || 'pending';
                            return (
                              <div
                                key={up.page_name}
                                style={{ width: '88px', cursor: 'pointer', borderRadius: '8px', overflow: 'hidden', border: `2px solid ${status === 'flagged' ? '#f48fb1' : status === 'redo' ? '#9fa8da' : status === 'approved' ? '#a5d6a7' : '#ddd'}`, backgroundColor: '#fff' }}
                                onClick={() => openReview(up)}
                              >
                                <img
                                  src={`${IMAGE_BASE}/${up.image_path}`}
                                  alt={up.page_name}
                                  style={{ width: '100%', height: '110px', objectFit: 'cover', display: 'block' }}
                                  onError={(e) => { e.target.style.display = 'none'; }}
                                />
                                <div style={{ padding: '4px 6px 6px' }}>
                                  <div style={{ fontSize: '10px', color: '#555', wordBreak: 'break-all', lineHeight: '1.3' }}>
                                    {up.page_name}
                                  </div>
                                  ...
                                  {status === 'redo' && up.upload_approval_note && (
