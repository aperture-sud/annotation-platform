...
import {
  getMaskingRequests, approveMaskingRequest, rejectMaskingRequest,
...
const REQ_STATUS = {
  pending:  { backgroundColor: '#f3e5f5', color: '#6a1b9a' },
...
export default function AdminMaskingPage() {
  ...
  async function handleApprove() {
    setBusy(true);
    try {
      const updated = await approveMaskingPage(modal.page_name);
  ...
  async function handleSendBack() {
    setBusy(true);
    try {
      const updated = await sendBackMaskingPage(modal.page_name, note);
      setPages(prev => prev.map(p => p.page_name === modal.page_name ? { ...p, ...updated } : p));
  ...
  async function handleApproveReq(id) {
    try {
      const updated = await approveMaskingRequest(id);
  ...
  async function handleRejectReq(id) {
    try {
      const updated = await rejectMaskingRequest(id);
      setRequests(prev => prev.map(r => r.id === id ? updated : r));
    } catch (e) { alert(e.response?.data?.detail || 'Failed.'); }
  }

  const pendingPages = pages.filter(p => p.masking_area === 'pending_approval').length;
  const pendingReqs  = requests.filter(r => r.status === 'pending').length;

  const groups = [
    { key: 'pending_approval', pages: pages.filter(p => p.masking_area === 'pending_approval') },
    { key: 'needs_rework',     pages: pages.filter(p => p.masking_area === 'needs_rework')     },
    { key: 'approved',         pages: pages.filter(p => p.masking_area === 'approved')         },
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f6f8' }}>
          ...
          <div style={{ backgroundColor: '#fff', borderRadius: '14px', padding: '22px', width: '100%', maxWidth: '560px', display: 'flex', flexDirection: 'column', gap: '14px', maxHeight: '90vh', overflowY: 'auto' }}
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px' }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '12px', color: '#888', marginTop: '3px' }}>masked by {modal.masked_by}</div>
              </div>
              {AREA_STYLE[modal.masking_area] && (
                <span style={{ fontSize: '11px', padding: '3px 10px', borderRadius: '10px', fontWeight: 600, backgroundColor: AREA_STYLE[modal.masking_area].bg, color: AREA_STYLE[modal.masking_area].color }}>
                  {AREA_STYLE[modal.masking_area].label}
                </span>
              )}
            </div>

            {modal.masking_review_note && (
              <div style={{ backgroundColor: '#fff8e1', borderRadius: '6px', padding: '10px 12px', fontSize: '12px', color: '#e65100' }}>
                <span style={{ fontWeight: 600 }}>Previous remark: </span>{modal.masking_review_note}
              </div>
            )}

            <img
              src={modal.masked_image_path ? `${MASKED_BASE}/${modal.masked_image_path}` : `${IMAGE_BASE}/${modal.image_path}`}
              alt={modal.page_name}
              style={{ width: '100%', borderRadius: '8px', border: '1px solid #e0e0e0' }}
              onError={e => { e.target.style.display = 'none'; }} />

            {modal.masking_area !== 'approved' && (
              <div>
                <label style={{ fontSize: '12px', color: '#666', display: 'block', marginBottom: '4px' }}>
                  Remark (optional — shown to masker on send back)
                </label>
                <textarea value={note} onChange={e => setNote(e.target.value)}
                  placeholder="e.g. Area not fully covered"
      ...
      <div style={{ padding: '28px', maxWidth: '860px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '20px', paddingBottom: '16px', borderBottom: '1px solid #e4e4e4' }}>
          ...
          <h1 style={{ fontSize: '18px', fontWeight: 700, color: '#1a1a1a', margin: '0 0 0 14px', flex: 1 }}>Masking</h1>
          ...
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              style={{
                ...
                borderBottom: tab === t.id ? '2px solid #6a1b9a' : '2px solid transparent',
                color: tab === t.id ? '#6a1b9a' : '#888',
                display: 'flex', alignItems: 'center', gap: '6px',
              }}>
              {t.label}
              {t.badge > 0 && (
                <span style={{ fontSize: '11px', padding: '1px 6px', borderRadius: '8px', fontWeight: 600, backgroundColor: tab === t.id ? '#f3e5f5' : '#f0f0f0', color: tab === t.id ? '#6a1b9a' : '#888' }}>
                  {t.badge}
                </span>
              )}
            </button>
        ...
        {tab === 'review' && (
          <div>
            {loading ? (
              ...
              <p style={{ color: '#bbb', fontSize: '13px', textAlign: 'center', padding: '40px 0' }}>No masking submissions yet.</p>
            ) : groups.map(g => {
              ...
              return (
                <div key={g.key} style={{ ...S.card, borderColor: as.border, marginBottom: '14px' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {g.pages.map(p => {
                      const imgSrc = p.masked_image_path ? `${MASKED_BASE}/${p.masked_image_path}` : `${IMAGE_BASE}/${p.image_path}`;
                      return (
                        <div key={p.page_name}
                          ...
                          <img src={imgSrc} alt={p.page_name}
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid #eee' }}>
                    {['Masker', 'Requested', 'Qty', 'Fulfilled', 'Status', ''].map(h => (
                ...
                <tbody>
                  {requests.map(r => (
                    <tr key={r.id} style={{ borderBottom: '1px solid #f5f5f5', backgroundColor: r.status === 'pending' ? '#fdf5ff' : 'transparent' }}>
                      ...
                      <td style={{ ...S.td, color: '#888' }}>{r.fulfilled}</td>
                      <td style={S.td}>
